...
def articles():
    ...
    #get geo argument from form in HTML
    geo = request.args.get("geo")
    
    #raise Error if argument is missing
    if not geo:
        raise RuntimeError("Geo not set")
    
    #search articles in that geo
    articles = lookup(geo)
    
    #return up to 5 articles as a JSON object
    if len(articles) > 5:
        return jsonify([articles[0], articles[1], articles[2], articles[3], articles[4], articles[5]])
    else:
