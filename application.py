...
@login_required
def index():
    #select each symbol owned by user
    portfolio_symbols = db.execute("SELECT shares, symbol FROM portfolio WHERE id = :id", \
                                    id = session["user_id"])
    #create variable to store total worth
    total_cash = 0
    
    #update symbols of price and total
...
def buy():
    ...
    if request.method == "GET":
        ...
        #ensure proper numbers of share
        try:
        ...
        #creating new stock if user not hasshares this symbol
        if not user_shares:
...
def history():
    ...
    histories = db.execute("SELECT * FROM histories WHERE id=:id", \
                            id=session["user_id"])
...
def sell():
    ...
    if request.method == "GET":
        ...
        #select symbol shares of the user
        user_shares = db.execute("SELECT shares FROM portfolio WHERE id=:id AND symbol=:symbol", \
                                id=session["user_id"], \
                                symbol=stock["symbol"])
        ...
        shares_total = user_shares[0]["shares"] - shares
        #delete shares from portfolio if shares equal 0
        if shares_total == 0:
            db.execute("DELETE FROM portfolio WHERE id=:id AND symbol=:symbol", \
                        id=session["user_id"],
                        symbol=stock["symbol"])
        
        #update portfolio shares count
        else:
            db.execute("UPDATE portfolio SET shares=:shares WHERE id=:id AND symbol=:symbol", \
                        shares=shares_total, \
                        id=session["user_id"], \
